...
extension UIViewController {
    ...
    func showLoading(imageNames: Array<UIImage>, timeInterval: Int) {
        cancelNotice()
...
class SwiftNotice: NSObject {
    ...
    static func showNoticeWithText(type: NoticeType,text: String?, autoClear: Bool, autoClearTime: Int) {
        ...
        let label = UILabel(frame: CGRectMake(25, 55,UIScreen.mainScreen().bounds.width - 100 ,0))
        label.text = text
        label.numberOfLines = 0
        label.font = UIFont.systemFontOfSize(13)
        label.textAlignment = NSTextAlignment.Center
        ...
        let superFrame = CGRectMake(0, 0, label.frame.width + 50 , label.frame.height + 66)
        window.frame = superFrame
        mainView.frame = superFrame
